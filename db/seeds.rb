...
departments = [
  ...
  { name: "業務部", code: "SAL" },
  { name: "人資部", code: "HR" }
].map { |d| Department.create!(d) }

puts "✓ 建立 #{departments.count} 個部門"
...
prizes1 = [
  ...
  event1.prizes.create!(
    ...
    drawn: true,
    drawn_at: event1.event_date + (data[:position] * 15).minutes
  )
end
...
prizes1.each do |prize|
  prize.quantity.times do
    ...
    Winner.create!(
      ...
      drawn_at: prize.drawn_at,
      distributed: [true, true, false].sample,
      distributed_at: [true, true, false].sample ? prize.drawn_at + 1.hour : nil
    )
  end
end
...
# ============================================================
# Event 2: 2026 春酒抽獎 (進行中，可 DEMO 開獎)
# ============================================================
puts "\n建立活動: 2026 春酒抽獎 (進行中)"
...
event2 = Event.create!(
  ...
  sms_template: "恭喜 {name}！您在春酒抽獎中獲得 {prize}，價值 ${value} 元。",
  email_template: "親愛的 {name}，\n\n恭喜您獲得 {prize}！\n獎項價值：${value} 元\n\n請至服務台領取。"
)

# Add 25 participants
participants[0..24].each { |p| event2.event_participants.create!(participant: p) }

# Create prizes with schedule (small to big)
...
puts "  ✓ #{event2.prizes.count} 個獎項 (未開獎), #{event2.event_participants.count} 位參與者"
puts "  ✓ 活動密碼: spring2026"

# ============================================================
# Event 3: 2026 尾牙抽獎 (草稿，可編輯)
# ============================================================
puts "\n建立活動: 2026 尾牙抽獎 (草稿)"
...
# Create prizes with eligibility rules (small to big)
...
[
  ...
  event3.prizes.create!(
    ...
    position: data[:position],
    scheduled_at: data[:scheduled_at],
    eligibility_rules: data[:eligibility],
    display_fields: ["name", "employee_id"],
    privacy_settings: { "name" => true, "employee_id" => false },
    drawn: false
  )
end
...
# ============================================================
# Event 4: 部門抽獎活動 (草稿，有部門限制)
# ============================================================
puts "\n建立活動: 部門抽獎活動 (草稿)"

event4 = Event.create!(
  name: "部門抽獎活動",
  event_date: DateTime.new(2026, 6, 15, 14, 0, 0),
  status: :draft,
...
# Add 20 participants
participants[0..19].each { |p| event4.event_participants.create!(participant: p) }

# Create prizes with department restrictions
[
  { name: "普獎 - 文具組", value: 200, quantity: 10, position: 1 },
  { name: "工程部專屬獎", value: 2000, quantity: 2, position: 2, eligibility: { "departments" => ["工程部"] } },
  { name: "設計部專屬獎", value: 2000, quantity: 2, position: 3, eligibility: { "departments" => ["設計部"] } },
  { name: "資深員工獎", value: 5000, quantity: 1, position: 4, eligibility: { "min_seniority" => 5 } }
].each do |data|
  event4.prizes.create!(
    name: data[:name],
    value: data[:value],
    quantity: data[:quantity],
    position: data[:position],
...
puts "  ✓ #{event4.prizes.count} 個獎項, #{event4.event_participants.count} 位參與者"
...
[
  { name: "標準中獎通知", notification_type: "winning", method: "sms",
    content: "恭喜 {name}！您在 {event_name} 中獲得 {prize}，價值 ${value} 元。請至服務台領取。", is_default: true },
  { name: "標準中獎通知", notification_type: "winning", method: "email",
    content: "親愛的 {name}，\n\n恭喜您在 {event_name} 中獲得 {prize}！\n獎項價值：${value} 元\n\n請至服務台領取。", is_default: true },
  { name: "標準發放通知", notification_type: "distribution", method: "sms",
    content: "{name} 您好：您的獎品 {prize} 已備妥，請至人資部領取。", is_default: true },
  { name: "標準發放通知", notification_type: "distribution", method: "email",
    content: "親愛的 {name}，\n\n您的獎品「{prize}」已準備就緒，請至人資部領取。\n\n謝謝！", is_default: true }
].each_with_index do |data, i|
  NotificationTemplate.create!(data.merge(position: i + 1))
end

puts "✓ 建立 #{NotificationTemplate.count} 個通知模板"
...
puts "\n【活動列表】"
puts "  1. #{event1.name}"
puts "     狀態: 已完成 | 參與者: #{event1.event_participants.count} 人 | 中獎: #{event1.winners.count} 人"
puts "  2. #{event2.name}"
puts "     狀態: 進行中 | 參與者: #{event2.event_participants.count} 人 | 密碼: spring2026"
puts "  3. #{event3.name}"
puts "     狀態: 草稿 | 參與者: #{event3.event_participants.count} 人 | 含年資條件"
puts "  4. #{event4.name}"
puts "     狀態: 草稿 | 參與者: #{event4.event_participants.count} 人 | 含部門限制"

puts "\n【參與者】#{Participant.count} 人"
puts "【部門】#{Department.count} 個"
puts
